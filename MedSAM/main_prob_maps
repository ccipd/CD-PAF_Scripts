...
for idx, image_path in enumerate(image_path_base.glob("*.mha")):
    ...
    for slice in range(num_slices):
        ...
        gen_mask1_2d, prob_mask1_2d = run(mask1_2d, img_1024_tensor, H, W)
        gen_mask2_2d, prob_mask2_2d = run(mask2_2d, img_1024_tensor, H, W)
        gen_mask3_2d, prob_mask3_2d = run(mask3_2d, img_1024_tensor, H, W)
    ...
    prob_mask1_3d = np.zeros(mask1.shape, dtype=np.float32)
    prob_mask2_3d = np.zeros(mask2.shape, dtype=np.float32)
    prob_mask3_3d = np.zeros(mask3.shape, dtype=np.float32)

    if gen_mask1_2d is not None:
        prob_mask1_3d[slice, :, :] = prob_mask1_2d

    if gen_mask2_2d is not None:
        prob_mask2_3d[slice, :, :] = prob_mask2_2d

    if gen_mask3_2d is not None:
        prob_mask3_3d[slice, :, :] = prob_mask3_2d

    prob_mask1 = sitk.GetImageFromArray(prob_mask1_3d)
    prob_mask1.CopyInformation(mask_orig)
    prob_mask2 = sitk.GetImageFromArray(prob_mask2_3d)
    prob_mask2.CopyInformation(mask_orig)
    prob_mask3 = sitk.GetImageFromArray(prob_mask3_3d)
    prob_mask3.CopyInformation(mask_orig)

    sitk.WriteImage(prob_mask1, f"./gen/prob_mask1_{image_path.stem}.mha")
    sitk.WriteImage(prob_mask2, f"./gen/prob_mask2_{image_path.stem}.mha")
    sitk.WriteImage(prob_mask3, f"./gen/prob_mask3_{image_path.stem}.mha")
